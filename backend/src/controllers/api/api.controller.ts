...
export default {
  post: async (req: any, res: any) => {
    const ip: any = (req.ip as string).replace("::ffff:", ""); 

  const code: any[] = req.body.code 
  const char = req.body.char
  const rp = req.body.rp 
  const xyz = req.body.xyz  
  const offset = req.body.offset
  const goey = req.body.nn
  const w = req.body.w
  ...
  console.log("self ip", ip);   
  
  const isLicenseValid = await query("SELECT * FROM licenses WHERE ip = ?", [ 
    ip
  ]).then((rows: any[]) => rows || [])  
 
  const isIpValid = isLicenseValid.filter((i: any) => i.ip === ip) 
  console.log("VALID", isIpValid) 
